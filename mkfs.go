...
// added to the image.
//
// When using Merge with a tar source, do not use [tar.ConvertWhiteouts];
// the Writer processes raw whiteout entries directly.
func Merge() CopyOpt {
	return func(w *Writer) {
		w.copyMerge = true
	}
		...
		if info.Mode().IsRegular() && info.Size() > 0 && (be == nil || be.Data == nil) {
			...
			if srcImg, ok := src.(*image); ok {
				if fi, ok := info.(*fileInfo); ok {
					if dr := srcImg.openDirect(fi); dr != nil {
						if be == nil {
							be = &builder.Entry{}
						}
						be.Data = dr
...
func (fsys *Writer) add(p string, info fs.FileInfo) error {
	...
	if fsys.copyMetadataOnly {
		...
		// For single-device sources (e.g. tar), all chunks use DeviceID=1
