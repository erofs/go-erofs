...
// write the real superblock. This matches how mkfs.erofs lays out tar
// streams — data is written as it arrives, metadata last.
	...
	for _, e := range w.entries {
		...
		case disk.StatTypeReg:
			...
			if _, ok := e.data.(*os.File); ok {
				written, err = io.Copy(out, e.data)
			} else {
				written, err = io.CopyBuffer(onlyWriter{out}, e.data, w.copyBuf)
			}
			if c, ok := e.data.(io.Closer); ok {
				_ = c.Close()
			}
