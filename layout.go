...
// direntName returns the name for the i-th dirent entry (0=".", 1="..", 2+=children).
func direntName(e *erofsEntry, i int) string {
	switch i {
	case 0:
		return "."
	case 1:
		return ".."
	default:
		return e.children[i-2].name
	}
}

func (w *erofsWriter) direntDataSize(e *erofsEntry) int {
	nEntries := len(e.children) + 2
	if len(e.children) == 0 {
	...
	for i < nEntries {
		...
		if i == start {
			blockUsed = disk.SizeDirent + len(direntName(e, i))
			i++
		}
